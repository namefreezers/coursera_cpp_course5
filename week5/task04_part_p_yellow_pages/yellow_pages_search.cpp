...
//            this->type_.has_value() && phone_to_check.type_.has_value() && *this->type_ != phone_to_check.type_) {
    ...
    bool CompanyPhoneConstraint::is_suite(const YellowPagesDatabase::Company &company_to_check) const {
        ...
        for (const YellowPagesDatabase::CompanyPhone& company_phone : company_to_check.get_company_phones()) {
            for (const OnePhoneConstraint &one_phone_constraint : phone_constraints_) {
                if (one_phone_constraint.is_one_phone_suite(company_phone)) {
                    return true;
                }
            }
        }
