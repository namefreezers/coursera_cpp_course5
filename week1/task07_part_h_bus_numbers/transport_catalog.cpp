...
TransportCatalog::TransportCatalog(vector<Descriptions::InputQuery> data, const Json::Dict &routing_settings_json, const Json::Dict &render_settings_json) {
    ...
    for (const auto&[stop_name, desc_stop] : stops_dict) {
        stop_coords_[stop_name] = desc_stop->position;
    }
    for (const auto&[bus_name, desc_bus] : buses_dict) {
        bus_stops_[bus_name] = desc_bus->stops;
    }
...
std::string TransportCatalog::RenderMap() const {
    PointConverter converter(stop_coords_, render_settings);
    Svg::Document doc;

    size_t color_idx = 0;
    for (const auto&[bus_name, bus_stops] : bus_stops_) {
        Svg::Polyline polyline;
        for (const auto &stop : bus_stops) {
            polyline.AddPoint(converter(stop_coords_.at(stop)));
        }
    ...
    for (const auto& [stop_name, coords] : stop_coords_) {
        Svg::Circle circle;
